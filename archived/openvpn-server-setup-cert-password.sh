...
trap 'err "Install failed on line $LINENO"; exit 1' ERR
...
sleep 2
systemctl is-active --quiet "openvpn-server@${SERVER_NAME}" || {
...
echo -e "${CYAN}Manage users:${NC} openvpn-manage add <user> | passwd <user> | remove <user> | list"
echo -e "${CYAN}Quick status:${NC} openvpn-status"
echo
warn "Next: create a user ->  sudo openvpn-manage add alice  ; then import /etc/openvpn/clients/alice.ovpn into the client."
