...
log "=== 6. Creating OpenVPN Chroot Directory Structure ==="
# Create chroot directory with proper structure
mkdir -p /var/lib/openvpn/tmp
mkdir -p /var/lib/openvpn/dev
mkdir -p /var/lib/openvpn/proc
mkdir -p /var/lib/openvpn/sys
mkdir -p /var/lib/openvpn/run
mkdir -p /var/lib/openvpn/etc/openvpn

# Set permissions
chmod 755 /var/lib/openvpn
chmod 755 /var/lib/openvpn/tmp
chmod 755 /var/lib/openvpn/dev
chmod 755 /var/lib/openvpn/proc
chmod 755 /var/lib/openvpn/sys
chmod 755 /var/lib/openvpn/run
chmod 755 /var/lib/openvpn/etc
chmod 755 /var/lib/openvpn/etc/openvpn

chown root:root /var/lib/openvpn
chown root:root /var/lib/openvpn/tmp
chown root:root /var/lib/openvpn/dev
chown root:root /var/lib/openvpn/proc
chown root:root /var/lib/openvpn/sys
chown root:root /var/lib/openvpn/run
chown root:root /var/lib/openvpn/etc
chown root:root /var/lib/openvpn/etc/openvpn

# Copy certificate files to chroot
cp /etc/openvpn/*.crt /var/lib/openvpn/etc/openvpn/ 2>/dev/null || true
cp /etc/openvpn/*.key /var/lib/openvpn/etc/openvpn/ 2>/dev/null || true
cp /etc/openvpn/*.pem /var/lib/openvpn/etc/openvpn/ 2>/dev/null || true
cp /etc/openvpn/ipp.txt /var/lib/openvpn/etc/openvpn/ 2>/dev/null || true

# Set permissions for copied files
chmod 600 /var/lib/openvpn/etc/openvpn/*.key 2>/dev/null || true
chmod 644 /var/lib/openvpn/etc/openvpn/*.crt 2>/dev/null || true
chmod 644 /var/lib/openvpn/etc/openvpn/*.pem 2>/dev/null || true
chmod 644 /var/lib/openvpn/etc/openvpn/ipp.txt 2>/dev/null || true

chown root:root /var/lib/openvpn/etc/openvpn/* 2>/dev/null || true

log "=== 7. Updating OpenVPN Configuration (No Chroot) ==="
# Update server.conf without chroot to avoid complexity
...
log "=== 9. Testing OpenVPN Configuration (No Test Flag) ==="
# Test OpenVPN configuration without --test flag
if openvpn --config /etc/openvpn/server.conf --help >/dev/null 2>&1; then
    success "✅ OpenVPN configuration syntax is valid"
else
