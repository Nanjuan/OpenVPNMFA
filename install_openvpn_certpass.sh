#!/usr/bin/env bash
# openvpn-cert-pass-installer.sh
# OpenVPN server with certificate-only auth and passphrase-protected keys (no PAM)
# Supports Ubuntu/Debian and RHEL/Rocky/Alma. Systemd required.
set -euo pipefail

### -------- CONFIG DEFAULTS (you can change before running) --------
OVPN_PORT_DEFAULT="1194"
OVPN_PROTO_DEFAULT="udp"
OVPN_NET="10.8.0.0"
OVPN_MASK="255.255.255.0"
OVPN_DNS_1="1.1.1.1"
OVPN_DNS_2="8.8.8.8"
SERVER_NAME="server"          # easy-rsa CN for server
PROFILE_REMOTE_IP_DEFAULT=""  # if empty, script will ask (e.g., 143.198.18.128)
ASKPASS_FILE="/etc/openvpn/server/server.pass"
SERVER_CONF="/etc/openvpn/server/server.conf"
EASYRSA_DIR="/etc/openvpn/easy-rsa"
PKI_DIR="$EASYRSA_DIR/pki"
TA_KEY="/etc/openvpn/ta.key"
CRL_FILE="$PKI_DIR/crl.pem"
OUTPUT_DIR="/root/openvpn-clients"  # where generated .ovpn files are written

### -------- Helpers --------
die(){ echo "ERROR: $*" >&2; exit 1; }
need_root(){ [ "$(id -u)" -eq 0 ] || die "Run as root."; }
detect_pkg(){
  if command -v apt-get >/dev/null 2>&1; then PKG="apt"; return; fi
  if command -v dnf >/dev/null 2>&1; then PKG="dnf"; return; fi
  if command -v yum >/dev/null 2>&1; then PKG="yum"; return; fi
  die "Unsupported distro: need apt, dnf, or yum."
}
pkg_install(){
  case "$PKG" in
    apt)
      apt-get update -y
      DEBIAN_FRONTEND=noninteractive apt-get install -y openvpn easy-rsa
      ;;
    dnf) dnf install -y openvpn easy-rsa || dnf install -y openvpn easy-rsa-3 ;;
    yum) yum install -y epel-release || true; yum install -y openvpn easy-rsa || yum install -y easy-rsa-3 ;;
  esac
}

prompt_default(){
  local prompt="$1" default="$2" var
  read -r -p "$prompt [$default]: " var
  echo "${var:-$default}"
}

ensure_dirs(){
  mkdir -p /etc/openvpn/server /etc/openvpn/tmp "$OUTPUT_DIR"
  chmod 700 "$OUTPUT_DIR"
}

enable_ip_forward(){
  sed -ri 's|^#?net.ipv4.ip_forward *=.*|net.ipv4.ip_forward=1|' /etc/sysctl.conf
  sysctl -p >/dev/null || true
}

ensure_easyrsa(){
  if [ ! -d "$EASYRSA_DIR" ]; then
    mkdir -p "$EASYRSA_DIR"
    # On some distros, easy-rsa provides the vars and scripts in /usr/share/easy-rsa
    if [ -d /usr/share/easy-rsa ]; then
      cp -a /usr/share/easy-rsa/* "$EASYRSA_DIR"/
    fi
  fi
}

init_pki(){
  cd "$EASYRSA_DIR"
  if [ ! -d "$PKI_DIR" ]; then
    ./easyrsa init-pki
  fi
}

build_ca(){
  cd "$EASYRSA_DIR"
  if [ ! -f "$PKI_DIR/private/ca.key" ]; then
    echo
    echo ">>> Building CA (you will be prompted for a CA key passphrase)"
    ./easyrsa build-ca
  fi
}

build_server(){
  cd "$EASYRSA_DIR"
  if [ ! -f "$PKI_DIR/issued/${SERVER_NAME}.crt" ]; then
    echo
    echo ">>> Building server cert/key (you will be prompted for a SERVER key passphrase)"
    ./easyrsa build-server-full "$SERVER_NAME"
  fi
  if [ ! -f "$PKI_DIR/dh.pem" ]; then
    ./easyrsa gen-dh
  fi
  if [ ! -f "$CRL_FILE" ]; then
    ./easyrsa gen-crl
  else
    ./easyrsa gen-crl || true
  fi
  if [ ! -f "$TA_KEY" ]; then
    openvpn --genkey secret "$TA_KEY"
    chmod 600 "$TA_KEY"
  fi
  # Make CRL readable by OpenVPN service
  install -m 0644 "$CRL_FILE" /etc/openvpn/server/crl.pem
}

write_server_conf(){
  local ip="$1" port="$2" proto="$3"
  cat > "$SERVER_CONF" <<EOF
# OpenVPN Server Configuration (PAM-free, cert-only with passphrase-protected keys)
port $port
proto $proto
dev tun
topology subnet

ca $PKI_DIR/ca.crt
cert $PKI_DIR/issued/${SERVER_NAME}.crt
key $PKI_DIR/private/${SERVER_NAME}.key
dh $PKI_DIR/dh.pem
tls-crypt $TA_KEY
crl-verify /etc/openvpn/server/crl.pem

server $OVPN_NET $OVPN_MASK
ifconfig-pool-persist ipp.txt

push "redirect-gateway def1 bypass-dhcp"
push "dhcp-option DNS $OVPN_DNS_1"
push "dhcp-option DNS $OVPN_DNS_2"

keepalive 10 120
data-ciphers AES-256-GCM
data-ciphers-fallback AES-256-GCM
auth SHA256
user nobody
group nogroup
persist-key
persist-tun

# If the server key is encrypted (recommended), use askpass
# The file will contain the server key passphrase (chmod 600)
askpass $ASKPASS_FILE

# Harden a bit
explicit-exit-notify 1
verb 3
EOF
}

set_askpass(){
  echo
  echo "The server's private key is encrypted. OpenVPN under systemd needs an askpass file."
  echo "Enter the SERVER KEY passphrase (stored root-only at $ASKPASS_FILE)."
  echo "If you prefer to type it manually every boot, leave empty and we will remove 'askpass'."
  read -r -s -p "Server key passphrase: " spass; echo
  if [ -n "$spass" ]; then
    mkdir -p "$(dirname "$ASKPASS_FILE")"
    printf '%s\n' "$spass" > "$ASKPASS_FILE"
    chmod 600 "$ASKPASS_FILE"
  else
    # Remove askpass line if no passphrase provided here
    sed -i '/^askpass /d' "$SERVER_CONF"
    echo "No askpass set; service start will fail if it cannot prompt. You can start in foreground manually."
  fi
}

start_service(){
  systemctl enable openvpn-server@"$SERVER_NAME" >/dev/null 2>&1 || true
  systemctl daemon-reload
  systemctl restart openvpn-server@"$SERVER_NAME"
  sleep 1
  systemctl --no-pager --full status openvpn-server@"$SERVER_NAME" || true
}

make_client(){
  local cn="$1"
  cd "$EASYRSA_DIR"
  if [ -f "$PKI_DIR/issued/${cn}.crt" ]; then
    echo "Client ${cn} already exists."
  else
    echo
    echo ">>> Creating client '$cn' (you will be prompted for a CLIENT key passphrase)"
    ./easyrsa build-client-full "$cn"
  fi
}

inline_ovpn(){
  local cn="$1" remote_ip="$2" port="$3" proto="$4"
  local out="$OUTPUT_DIR/${cn}.ovpn"
  cat > "$out" <<EOF
client
dev tun
proto $proto
remote $remote_ip $port
resolv-retry infinite
nobind
persist-key
persist-tun

remote-cert-tls server
cipher AES-256-GCM
auth SHA256
verb 3

<ca>
$(cat "$PKI_DIR/ca.crt")
</ca>

<cert>
$(openssl x509 -in "$PKI_DIR/issued/${cn}.crt")
</cert>

<key>
$(cat "$PKI_DIR/private/${cn}.key")
</key>

<tls-crypt>
$(cat "$TA_KEY")
</tls-crypt>
EOF
  chmod 600 "$out"
  echo "Generated: $out"
}

rebuild_crl(){
  cd "$EASYRSA_DIR"
  ./easyrsa gen-crl
  install -m 0644 "$CRL_FILE" /etc/openvpn/server/crl.pem
  systemctl restart openvpn-server@"$SERVER_NAME"
}

revoke_client(){
  local cn="$1"
  cd "$EASYRSA_DIR"
  ./easyrsa revoke "$cn" || true
  rebuild_crl
  echo "Revoked $cn and refreshed CRL."
}

list_clients(){
  if [ -f "$PKI_DIR/index.txt" ]; then
    echo "Status  Expiry(UTC)          Serial            CN"
    awk '/^V|^R/{
      status=$1=="V"?"VALID":"REVOKED";
      # Columns: status, notBefore, notAfter, serial, filename, unknown, cn
      # Easy-RSA index format: status expiry revocation serial filename "subject"
      # We print: status, expiry, serial, CN
      split($NF, a, "/CN="); cn=a[length(a)];
      printf "%-7s %-20s %-16s %s\n", status, $2, $4, cn
    }' "$PKI_DIR/index.txt" | sort
  else
    echo "No PKI index found."
  fi
}

show_menu(){
  cat <<'MENU'

===== OpenVPN Cert+Passphrase Manager =====
1) Add new client (encrypted key, inline .ovpn)
2) Revoke client
3) List clients
4) Restart OpenVPN service
5) Show service status
6) Exit
MENU
}

### ---------------- MAIN FLOW ----------------
need_root
detect_pkg
ensure_dirs
enable_ip_forward
ensure_easyrsa
pkg_install

# Ask for public IP/port/proto once
PROFILE_REMOTE_IP="${PROFILE_REMOTE_IP_DEFAULT}"
if [ -z "$PROFILE_REMOTE_IP" ]; then
  PROFILE_REMOTE_IP=$(prompt_default "Public IP or DNS for clients to connect" "$(hostname -I | awk '{print $1}')")
fi
OVPN_PORT=$(prompt_default "OpenVPN port" "$OVPN_PORT_DEFAULT")
OVPN_PROTO=$(prompt_default "OpenVPN protocol (udp/tcp)" "$OVPN_PROTO_DEFAULT")

# Initialize PKI and build materials
init_pki
build_ca
build_server
write_server_conf "$PROFILE_REMOTE_IP" "$OVPN_PORT" "$OVPN_PROTO"
set_askpass
start_service

echo
echo "Base setup complete."
echo "Clients will authenticate with their certificate; each private key is encrypted and prompts for passphrase on connect."
echo "Client profiles (.ovpn) will be written to: $OUTPUT_DIR"
echo

# Management loop
while true; do
  show_menu
  read -r -p "Select: " choice
  case "$choice" in
    1)
      read -r -p "Client name (CN): " CN
      [ -z "$CN" ] && { echo "No CN provided."; continue; }
      make_client "$CN"
      inline_ovpn "$CN" "$PROFILE_REMOTE_IP" "$OVPN_PORT" "$OVPN_PROTO"
      ;;
    2)
      read -r -p "Client name (CN) to revoke: " CN
      [ -z "$CN" ] && { echo "No CN provided."; continue; }
      revoke_client "$CN"
      ;;
    3) list_clients ;;
    4) systemctl restart openvpn-server@"$SERVER_NAME"; systemctl --no-pager --full status openvpn-server@"$SERVER_NAME" || true ;;
    5) systemctl --no-pager --full status openvpn-server@"$SERVER_NAME" || true ;;
    6) echo "Done."; exit 0 ;;
    *) echo "Invalid choice." ;;
  esac
done
