...
usage() {
  cat <<'EOF'
Usage:
  networking-setup.sh [options]

General:
  -h, --help                      Show this help and exit
  -d, --daemon                    Non-interactive mode (no prompts). Requires --action.

Actions (match the interactive menu):
  --action full                   Run: Full network setup (forwarding + NAT + persist)
  --action add-route              Run: Add new route (additional VPN CIDR / WAN iface)
  --action autostart              Run: Enable OpenVPN autostart on reboot
  --action status                 Run: Show OpenVPN service status
  --action inspect                Run: Show current network configuration

Parameters:
  --vpn-cidr CIDR                 e.g., 10.8.0.0/24
                                  • optional for --action full (overrides detection)
                                  • REQUIRED for --action add-route
  --wan IFACE                     e.g., eth0
                                  • optional for --action full (overrides detection)
                                  • REQUIRED for --action add-route

Overrides (optional):
  --instance NAME                 OpenVPN systemd instance name (default: server)
  --server-conf PATH              Path to server.conf (default: /etc/openvpn/server/<instance>.conf)

Examples:
  # Interactive menu (unchanged behavior)
  sudo ./networking-setup.sh

  # Full setup non-interactively (auto-detects CIDR and WAN)
  sudo ./networking-setup.sh -d --action full

  # Full setup with explicit values
  sudo ./networking-setup.sh -d --action full --vpn-cidr 10.9.0.0/24 --wan ens3

  # Add an additional routed network
  sudo ./networking-setup.sh -d --action add-route --vpn-cidr 10.10.0.0/24 --wan eth0

  # Enable autostart
  sudo ./networking-setup.sh -d --action autostart
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --vpn-cidr) VPN_CIDR_FLAG="${2:-}"; shift 2 ;;
...
# Apply optional overrides (do not change default behavior otherwise)
...
if $DAEMON; then
  ...
  case "$ACTION" in
    full)
      if [[ -n "$VPN_CIDR_FLAG" || -n "$WAN_IFACE_FLAG" ]]; then
        # Run full setup but override detections when flags provided
        enable_ip_forward
        ensure_persist_tools
        vpn_cidr="${VPN_CIDR_FLAG:-$(detect_vpn_cidr_from_conf "$SERVER_CONF")}"
        [[ -z "$vpn_cidr" ]] && vpn_cidr="10.8.0.0/24"
        wan="${WAN_IFACE_FLAG:-$(detect_public_iface)}"
        [[ -z "$wan" ]] && wan="eth0"
        echo "[*] Using VPN subnet: $vpn_cidr"
        echo "[*] Using WAN iface : $wan"
        add_nat_rules "$vpn_cidr" "$wan"
        persist_rules
        echo "==== Network setup complete (daemon) ===="
      else
        full_network_setup
      fi
      exit 0
      ;;
    add-route)
      require "--vpn-cidr" "$VPN_CIDR_FLAG"
      require "--wan" "$WAN_IFACE_FLAG"
      enable_ip_forward
      ensure_persist_tools
      add_nat_rules "$VPN_CIDR_FLAG" "$WAN_IFACE_FLAG"
      persist_rules
      echo "==== Route added and persisted (daemon) ===="
      echo "VPN subnet : $VPN_CIDR_FLAG"
      echo "WAN iface  : $WAN_IFACE_FLAG"
      exit 0
      ;;
    autostart)
      configure_autostart
