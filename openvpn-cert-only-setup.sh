...
# Matches the UI/format of our previous installers, but keeps your original behavior:
# - CA/SERVER/CLIENT keys created with *nopass*
# - No NAT/UFW/iptables here (networking handled separately)
...
SERVER_DIR="/etc/openvpn/server"                        # OpenVPN 2.6 systemd instance dir
...
# Server config parameters (same defaults as your script)
SERVER_NAME="server"
...
# Security/ciphers (kept aligned with your script)
AUTH="SHA512"
TLS_VERSION="1.2"
...
update_system(){
  ...
  DEBIAN_FRONTEND=noninteractive apt install -y curl wget gnupg2 software-properties-common ca-certificates \
                                                 lsb-release apt-transport-https openvpn easy-rsa
  # Helpful but harmless tools
  DEBIAN_FRONTEND=noninteractive apt install -y fail2ban unattended-upgrades
  success "System packages installed/updated"
}
...
setup_easyrsa(){
  ...
  # Your script used nopass for CA and server—keeping that behavior
...
configure_openvpn(){
  ...
  # server.conf (OpenVPN 2.6 systemd looks in /etc/openvpn/server/)
...
# Push some common options (you can adjust later)
