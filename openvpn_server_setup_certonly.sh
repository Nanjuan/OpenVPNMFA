...
# OpenVPN Server Setup Script - Certificate Only Authentication
# Compatible with Ubuntu 24.04+ and OpenVPN 2.6.12+
# Uses certificate-based authentication only (no username/password)

set -euo pipefail

# Script metadata
SCRIPT_VERSION="2.0-cert"
...
show_header() {
    header "OpenVPN Server Setup Script v$SCRIPT_VERSION (Certificate Only)"
    echo -e "${CYAN}Authentication: Certificate Only${NC}"
...
configure_openvpn() {
    ...
    # Create server configuration file (Certificate-only authentication)
    cat > $OPENVPN_DIR/$SERVER_NAME.conf << EOF
# OpenVPN Server Configuration
# Generated by openvpn-server-setup-cert-only.sh
...
# OpenVPN Management Script v2.0 (Certificate Only)
...
show_usage() {
    echo -e "${PURPLE}OpenVPN Management Script v2.0 (Certificate Only)${NC}"
    echo ""
    echo "Usage: $0 [command] [username]"
    echo ""
    echo "Commands:"
    echo "  add <username>     - Add a new VPN user (certificate only)"
...
# OpenVPN Status Script (Certificate Only)
...
echo "Authentication: Certificate Only"
...
install_openvpn_server() {
    header "Starting OpenVPN Server Installation (Certificate Only)"
    ...
    success "OpenVPN server is now running (Certificate Only)"
    info "Server IP: $(curl -s ifconfig.me)"
    info "VPN Port: $VPN_PORT"
    info "VPN Network: $VPN_NETWORK/24"
    info "Authentication: Certificate Only"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    case "${1:-install}" in
        ...
        *)
            ...
            echo ""
            echo "Commands:"
            echo "  install         - Install and configure OpenVPN server (Certificate Only)"
