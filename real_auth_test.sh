...
# Real OpenVPN Authentication Test
# This script properly tests password + MFA authentication
...
log "Testing REAL authentication for user: $USERNAME"
echo ""
...
log "Testing authentication with PAM..."

# Create a temporary test script
cat > /tmp/test_auth.sh << 'EOF'
#!/bin/bash
# Test script for PAM authentication
echo "$1" | /usr/lib/x86_64-linux-gnu/openvpn/plugins/openvpn-plugin-auth-pam.so openvpn
EOF

chmod +x /tmp/test_auth.sh

# Test the authentication
echo "$password_mfa" | /usr/lib/x86_64-linux-gnu/openvpn/plugins/openvpn-plugin-auth-pam.so openvpn 2>&1
AUTH_RESULT=$?
